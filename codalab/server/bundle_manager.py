...
class BundleManager(object):
    ...
    def _compute_request_disk(self, bundle, user_info):
        ...
        if not bundle.metadata.request_disk:
            return min(
                self._model.get_user_disk_quota_left(bundle.owner_id, user_info) - 1, self._max_request_disk
            )
    ...
    def _compute_request_time(self, bundle, user_info):
        ...
        if not bundle.metadata.request_time:
            return min(
                self._model.get_user_time_quota_left(bundle.owner_id, user_info) - 1, self._max_request_time
            )
